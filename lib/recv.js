...
let Envelope, Content, PreKeyBundleMessage
let UnidentifiedSenderMessage, SenderCertificate
...
protobuf.load('external/protos/SignalService.proto', function(err, signalRoot) {
  if (err) console.error('proto err', err)
  Envelope = signalRoot.lookupType('Envelope')
  Content = signalRoot.lookupType('Content')
  PreKeyBundleMessage = signalRoot.lookupType('PreKeyBundleMessage')
})
protobuf.load('external/protos/UnidentifiedDelivery.proto', function(err, uniddelRoot) {
  if (err) console.error('proto err', err)
  UnidentifiedSenderMessage = uniddelRoot.lookupType('UnidentifiedSenderMessage')
  SenderCertificate = uniddelRoot.lookupType('SenderCertificate')
})
...
async function handleUnidentifiedMessageType(env, ourKeypair) {
  ...
  // decode remainder
  const wrapper = UnidentifiedSenderMessage.decode(env.content.slice(1))
  ...
  const message = UnidentifiedSenderMessage.Message.decode(
  ...
  if (type === 101) {
    ...
    if (content.dataMessage) {
      ...
      if (content.dataMessage.body) {
        return content.dataMessage
      } else {
        console.warn('spurious iOS message?', content)
      }
    }
    //const preKeyMsg = PreKeyBundleMessage.decode(plaintext)
...
function handleMessage(msg, ourKeypair) {
  ...
  // handle ws/wsr
  const wsMessage = WebSocketMessage.decode(buf)
...
*/
  ...
  const env = Envelope.decode(wsMessage.request.body)
  /*
Envelope {
type: 6,
...
async function checkBox(pubKey, swarmUrl, ourKeypair, inLasthash, lib) {
  ...
  const newMsgs = await Promise.all(messageData.messages.map(async msg => {
    /*
     { data:
