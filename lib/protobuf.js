...
function encodeContentMessage(text, ts, options) {
  ...
  if (options && (options.displayName || options.avatar)) {
    ...
    if (options.avatar) {
      ...
      // used to be profile.avatar

      // this is deprecated 230120
      profile.profilePicture = options.avatar.url
      // this is the current method 230120
...
function decodeContentMessage(plaintext, id) {
  ...
  try {
    const unpaddedPlaintext = unpad(plaintext, id)
    const content = module.exports.Content.decode(new Uint8Array(unpaddedPlaintext))
    return content
  } catch (e) {
    console.log('protobuf::decodeContentMessage - decoding failure on', id, e)
  }
}
